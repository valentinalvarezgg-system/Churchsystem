...
router.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (payment.status === 'approved') {
      ...
      const user = await pgOne('SELECT "iglesiaId" FROM "User" WHERE "id"=$1 AND "deletedAt" IS NULL LIMIT 1', [Number(userId)])
      ...
      logger.info({ plan: planKey, userId, paymentId: payment.id }, 'Pago aprobado')
    }
  } catch (err) {
